...
import {fetchBulkAddTagsToTransport} from "app/redux/actions/transports";
import {SearchQuery} from "app/redux/reducers/searches";

type BulkAddTagsModalProps = {
    selectedTransportsQuery: SearchQuery;
    selectedTransportsCount: number;
    onClose: () => void;
};

export const BulkAddTagsModal: FunctionComponent<BulkAddTagsModalProps> = ({
    ...
    async function handleSubmit() {
        startLoading();
        dispatch(await fetchBulkAddTagsToTransport(tags, selectedTransportsQuery));
        stopLoading();
        onClose();
    }
    ...
    return (
        <Modal
            title={t("bulkAction.addTags.modalTitle")}
            id="bulk-add-tags-modal"
            onClose={onClose}
            mainButton={{
                children: t("bulkAction.addTags.modalConfirmButton"),
        ...
        >
            {!isLoading && (
                <>
                    ...
                    <Text my={4}>{t("bulkAction.addTags.informationText")}</Text>
