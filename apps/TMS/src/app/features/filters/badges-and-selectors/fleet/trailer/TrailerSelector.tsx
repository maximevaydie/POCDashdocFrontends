...
export function TrailerSelector({query, updateQuery, sortAndFilters}: Props) {
    ...
    return (
        <>
            <FilteringHeader dataTypeLabel={t("common.trailer")} conditionLabel={t("filter.in")} />
            <FilteringPaginatedListSelector<Trailer>
                fetchItemsUrl="trailers/"
                searchQueryKey="text"
                ...
                values={query.trailer__in ?? []}
                onChange={(value) => updateQuery({trailer__in: value})}
            />
        </>
    );
}
