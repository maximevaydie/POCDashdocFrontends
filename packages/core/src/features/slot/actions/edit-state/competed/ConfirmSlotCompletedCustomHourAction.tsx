...
export function ConfirmSlotCompletedCustomHourAction({slot, onAction}: Props) {
    const dispatch = useDispatch();
    const disabled = useSelector(isUpdating);
    const site = useSelector(selectSite);
    let limitDate: TzDate | undefined;
    if (site) {
        const date = (site.use_slot_handled_state && slot.handled_at) || slot.arrived_at;
        if (date) {
        ...
        <>
            ...
            {show && (
                <ConfirmSlotCustomHourModal
                    actionLabel={t("common.confirmEndOfActivity")}
                    onSubmit={action}
                    onClose={setHide}
                    limitDate={limitDate}
                />
    ...
    async function action(timestamp: string) {
        try {
            const actionResult = await dispatch(completeSlot({id: slot.id, timestamp}));
